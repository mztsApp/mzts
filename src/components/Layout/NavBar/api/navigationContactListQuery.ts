...
export const navigationContactListQuery = async () => {
  let isPending: boolean = true;
  let isError: boolean = false;
  let data: Data = null;

  try {
    const response = await fetch(generateEntryQuery('7pw8llgEy4f2lB5JZJInz4'), {
      cache: 'force-cache',
      next: {
        revalidate: Infinity,
      },
    ...
    const objectWithIds = Object.entries(resolvedRestItems).map(
      ([key, currentValue]) => [
        key,
        resolvedItemsFields.find((field) => field.sysId === currentValue.sys.id)
          ?.fields,
      ],
    );
